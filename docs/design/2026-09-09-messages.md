...
`org.lattejava.web.internal.MessageStore` holds one `ConcurrentHashMap` from relative file name to `Properties`. Each file is read on first use and kept for the life of the server; a missing file is remembered as missing. The store lives in `HTTPContext` attributes under the key `org.lattejava.web.Messages`, created on first use with `computeIfAbsent` (the attribute map is synchronized). A new `Web` has a new context and therefore a new cache.
