...
- **Breaking:** `Web` is now a `Configurable` class, which is the same interface that `HTTPServer` implements. This makes it fully configurable directly. However, this caused the `start(HTTPListener...)` method to be removed. 

## [0.11.0] - 2026-09-10

### Changed

- **Breaking:** `Messages` reads from `<baseDir>/messages` instead of `<baseDir>/web/messages`. `Web.baseDir(Path)` now names the same directory for messages and for files served by `files()`, which is the application's `web` directory.
