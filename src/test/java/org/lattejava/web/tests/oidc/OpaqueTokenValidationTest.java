...
/**
 * Exercises the opaque-access-token path ({@code validateAccessToken=false}), where {@link Authenticated} validates the
 * token at the IdP's RFC 7662 introspection endpoint rather than decoding it locally. The introspection response is the
 * claims source and supplies the {@code aud} claim for the audience check against the configured client id.
 */
public class OpaqueTokenValidationTest extends BaseOIDCTest {
  private static final int MOCK_PORT = 9099;

  private static OIDC<?> opaqueOIDC;

  @BeforeClass
  public static void setupOIDC() {
    // FusionAuth's discovery document does not advertise introspection_endpoint, so it is set explicitly here (the same
    // pattern APIAuthenticatedTest uses for the API flow).
  ...
  public void opaqueMode_audienceMismatch_redirectsToLogin() throws Exception {
    try (MockIdP mock = new MockIdP(MOCK_PORT)) {
      ...
      try (var web = new Web()) {
        ...
        HttpResponse<String> res = get("/protected/page?" + Authenticated.CSR_REDIRECT_PARAM + "=1", "access_token=any-opaque-token");
