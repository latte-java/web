...
/**
 * A literal-segment trie that maps path prefixes to middlewares. Installation appends to a node's
 * middleware list; collection walks from root along a request's segments, accumulating middlewares
 * at each node visited. This yields outer-to-inner ordering implicitly and {@code O(depth)} lookup
 * per request, with no sort and no entry-set scan.
 * <p>
 * Prefixes are treated as literal segments only — no {@code {name}} parameter support, which
 * matches how {@link org.lattejava.web.Web#install(Middleware...)} scoping is defined.
 *
...
public class MiddlewareTrie {
  ...
  /**
   * Installs middlewares at the given path prefix. The prefix is parsed via
   * {@link PathParser#parsePath(String)}; an empty or root-equivalent prefix installs at the root.
   *
  ...
  /**
   * Collects middlewares that apply to the given request segments. Walks from the root along the
   * segments, appending middlewares from each node visited. Stops at the first segment with no
   * matching child.
   *
